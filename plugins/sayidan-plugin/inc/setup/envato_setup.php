...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_globals() {
			...
			$this->page_slug = apply_filters( 'sayidan_theme_setup_wizard_page_slug', 'sayidan-setup' );
		...
		public function init_actions() {
			...
			// echo "aaa".$this->step; die;

			 	if ( false != get_option('sayidan_install_url', false) && $this->page == 'wc-setup' ){
			// // 	$this->step = isset( $_GET['step'] ) ? sanitize_key( $_GET['step'] ) : current( array_keys( $this->steps ) );
			// 		echo "aa";die;

wp_safe_redirect( get_site_url().get_option('sayidan_install_url', false), $status );
exit;
			 		// 	echo get_site_url().get_option('sayidan_install_url', false);
						// die;
			 	}
				//add sayidan admin menu
			    add_action('admin_menu', 'sayidan_plugin_setup_menu');
			     
			    function sayidan_plugin_setup_menu(){
			            add_menu_page( 'sayidan', 'Sayidan', 'manage_options', 'sayidan-menu', 'sayidan_menu_init', '', 2 );
			            add_submenu_page( 'sayidan-menu', esc_attr( 'Customize', 'Sayidan' ), esc_attr( 'Customize', 'sayidan' ), 'administrator', 'customize.php?theme=sayidan' );
			            add_submenu_page( 'sayidan-menu', esc_attr( 'Setup Wizard', 'Sayidan' ), esc_attr( 'Setup Wizard', 'sayidan' ), 'administrator', 'sayidan-setup', 'sayidan_menu_init' );
			    }

			     //http://wordpress.sayidan/wp-admin/customize.php?theme=sayidan&return=%2Fwp-admin%2Fthemes.php
			    function sayidan_menu_init(){
echo "<h1>Latest Improvements</h1>";
echo "<code>Version 1.6.2<br><br>
+ new: Page Builder integration<br>
+ new: add social icons in header<br>
+ new: setup Wizard integration<br>
+ new: social login possibility<br>
+ update: events can be categorized<br>
+ update: more translate friendly<br>
+ update: child theme improved<br><br>

Version 1.6.1<br><br>
+ update: extended contacts maps capabilities and styling<br>
+ update: visual bug fixing<br>
+ update: block not found problem fix<br><br>

Version 1.1.0<br><br>

+ fix filter in the directory page, it only searched by people’s name. <br>
+ jquery.syotimer.js translations added<br>
+ Auto slide slider parameters added<br>
+ Directory form registration added<br>
+ Added Google map api key to Customiser<br>
+ Contacts example page added<br>
+ Fix logo left and menu overlap problem<br>
+ Demo content import fix<br>
+ Vulnerable query fixed in block.php file<br>
+ Select first upcoming event or provide event ID in [sayidan_event] shortcode<br>
+ Favicon added<br>
+ Twitter plugin demo content fix<br>
+ Added support for Ultimate Member plugin</code>";
			    }
			}
		...
		public function admin_redirects() {
			...
			// if(sayidan_is_upgrading()){
			// 	wp_safe_redirect( admin_url('admin.php?page=sayidan-panel') );
			// } else {
		...
		public function admin_menus() {
			add_submenu_page('sayidan-panel', __( 'Setup Wizard','envato_setup' ), __( 'Setup Wizard','envato_setup' ), 'manage_options', $this->page_slug,  array( $this, $this->page_slug) );
		}
		...
		public function setup_wizard() {
			...
			update_option('sayidan_install_url', $_SERVER['REQUEST_URI']);
		...
		<h1 id="wc-logo">
			<?php
				$image_url = do_shortcode(get_theme_mod( 'site_logo', get_template_directory_uri().'/images/install-logo.png')); 
...
//				print_r($categories);
				...
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.' ), wp_get_theme()); ?></h1>
				<p class="lead success"><?php _e('It looks like you already have setup sayidan.');?></p>
			
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php _e( 'Run Setup Wizard Again' ); ?></a>
					<a href="<?php echo admin_url( 'admin.php?page=sayidan-panel' ); ?>"
					   class="button button-large"><?php _e( 'Exit to sayidan Panel' ); ?></a>
				</p>
				<?php
			} else {
				?>
...
*/
		...
		public function envato_setup_default_plugins() {
			...
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p class="lead"><?php _e( 'This will install the default plugins included with sayidan. You can add and remove plugins later on from within WordPress.', 'envato_setup' ); ?></p>
		...
		private function _make_child_theme( $new_theme_title ) {
				...
				$parent_theme_title = 'sayidan';
				$parent_theme_template = 'sayidan';
			...
			<form method="post">
				...
				<table>
					<tr>
						<td>
							<div id="current-logo">
								<?php $image_url = do_shortcode(get_theme_mod( 'site_logo', get_template_directory_uri().'/images/install-logo.png')); 
								$image_url = apply_filters('envato_setup_logo_image',$image_url);
								if ( $image_url ) {
...
-->
		...
		public function envato_setup_updates() {
			...
			if(isset($_POST[$slug . '_wup_purchase_code'])){
				...
				$homepage = file_get_contents('http://themesapi.kenzap.com/data.php?cmd=verify_code&code='.$temp_code.'&domain='.get_site_url());
			...
			<p class="lead">Enter your purchase code to enable all theme features and access support.</p>
				<?php
				    ...
				    if ( ! empty( $temp_code ) ) {
				      if ( ! $response_obj->{'success'} ) {
				        ...
				        //this means a valid purchase code is present and no errors were found
				       	echo '<div class="notice-success notice-alt notice-large" style="margin-bottom:15px!important">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy sayidan theme and its features.' ) . '</div>';
				      }
				    }
		...
		?>
			...
			<h1>Setup sayidan Child Theme (Optional)</h1>
			...
			<?php
				...
				$theme = get_option('fl_has_child_theme') ? wp_get_theme(get_option('fl_has_child_theme') )->Name : 'sayidan Child';
			 ?>
		...
		public function envato_setup_help_support() {
			...
			<ul>
				<li>Customization services (this is available through <a href="https://studio.envato.com/explore/websites-programming" target="_blank">Envato Studio</a>)</li>
				<li>Installation services (this is available through <a href="http://cloud.kenzap.com/" target="_blank">Kenzap Cloud</a>)</li>
		...
		public function envato_setup_ready() {
			...
			<div class="envato-setup-next-steps">
				...
				<div class="envato-setup-next-steps-last">
					...
					<ul>
						<li class="documentation"><a target="_blank" href="http://sayidan_docs.kenzap.com"><?php _e( 'Theme Documentation', 'envato_setup' ); ?></a></li>
			...
			<?php
			update_option('sayidan_install_url', false);
		}
